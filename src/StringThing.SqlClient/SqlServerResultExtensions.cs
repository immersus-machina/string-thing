...
public static class SqlServerResultExtensions
{
    ...
    private static int[] ResolveOrdinals<T>(DbDataReader reader, SqlServerSql statement)
        where T : IStringThingRow<T>
    {
    ...
    private static List<T> ReadAll<T>(DbDataReader reader, SqlServerSql statement)
        where T : IStringThingRow<T>
    {
        var ordinals = ResolveOrdinals<T>(reader, statement);
        var list = new List<T>();
    ...
    private static async Task<List<T>> ReadAllAsync<T>(DbDataReader reader, SqlServerSql statement, CancellationToken cancellationToken)
        where T : IStringThingRow<T>
    {
        var ordinals = ResolveOrdinals<T>(reader, statement);
        var list = new List<T>();
